# 👋 Hey, I'm Shahaf Sadon

🎓 I'm a first-year Software Engineering student at Bar-Ilan University  
💡 Passionate about learning through building real-world projects  
💻 Experienced with Java, Python and C, especially in object-oriented design and algorithmic thinking

## 🔧 Skills & Tools
- Java • Python • C • Git • IntelliJ • OOP • Algorithms

## 🚀 What I'm working on
I'm currently organizing and publishing selected university projects — including:
- 🎮 A full-featured Arkanoid game built step-by-step in Java
- 📊 Algorithms and logic in Python & C
- 🧠 Clean architecture, modular design and code documentation

## 📫 Contact
Feel free to reach out by email: shahaf.sadon@email.com  
Or just follow my journey here on GitHub 🚀
